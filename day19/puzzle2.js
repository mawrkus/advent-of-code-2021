...
function buildMap() {
  ...
  const map = referenceScannerBeacons.reduce(
    (acc, beacon) => ({ ...acc, [beacon]: beacon }),
    {}
  );
  ...
  while (remaingScannerBeacons.length) {
    for (let i = 0; i < remaingScannerBeacons.length; i += 1) {
      ...
      const overlap = findOverlap(map, beacons);
...
const { scannerPositions } = buildMap();
...
for (let i = 0; i < scannerPositions.length; i += 1) {
  for (let j = i + 1; j < scannerPositions.length; j += 1) {
    ...
    const distance = diff.reduce((acc, value) => Math.abs(value) + acc, 0);
