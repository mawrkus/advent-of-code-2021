...
const topInput = textInput
  ...
  .filter(Boolean)
  .map((rawLine) => {
    const firstLine = [...rawLine].map(Number);
    let line = firstLine;

    for (let i = 0; i < 4; i += 1) {
      line = line.concat([...firstLine].map((risk) => 1 + (risk + i) % 9));
    }
...
for (let i = 0; i < 4; i += 1) {
  for (let y = 0; y < topInput.length; y += 1) {
    inputChunk[y] = new Array(topInput[y].length);

    for (let x = 0; x < topInput[y].length; x += 1) {
      inputChunk[y][x] = 1 + (topInput[y][x] + i) % 9;
    }
  }

  input = input.concat(inputChunk);
}
